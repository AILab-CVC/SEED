...
def create_clip_vit_B(img_size=224,use_checkpoint=False,precision="fp16"):
    ...
    clip_model = torch.jit.load(cached_file, map_location="cuda").eval()
    ...
    for key in state_dict:
        if 'visual' in key:
            ...
            vision_state_dict[new_key] = state_dict[key]
    #interpolate_pos_embed(model,state_dict)
    
    incompatible_keys = model.load_state_dict(vision_state_dict, strict=False)
    print(incompatible_keys)
    
    if precision == "fp16":
        convert_weights_clip(model)
    return model
