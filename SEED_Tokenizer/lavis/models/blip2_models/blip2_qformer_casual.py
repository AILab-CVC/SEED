...
@registry.register_model("blip2_casual")
class Blip2QformerCasual(Blip2Base):
    """
    ...
    def forward(self, samples):
        ...
        # the last token as global visual token
        image_feats = image_feats[:,-1].unsqueeze(1).contiguous()
        ...
        if self.matching:
            ...
            vl_embeddings = output_itm.last_hidden_state[:, : query_tokens_itm.size(1), :][:,-1]
            vl_output = self.itm_head(vl_embeddings)
            logits = vl_output
            #print('vl_embeddings', vl_embeddings.shape, logits.shape)
