...
class Blip2ForConditionalGenerationModelOutput(ModelOutput):
    ...
    def to_tuple(self) -> Tuple[Any]:
        return tuple(self[k] if k not in
                     ["vision_outputs", "qformer_outputs", "language_model_outputs"] else getattr(self, k).to_tuple()
                     for k in self.keys())
...
class Blip2Model(Blip2PreTrainedModel):
    ...
    def forward(
        ...
        language_model_attention_mask = torch.ones(language_model_inputs.size()[:-1],
                                                   dtype=torch.long,
...
class Blip2ForConditionalGeneration(Blip2PreTrainedModel):
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        language_model_inputs = self.language_projection(query_output)
        language_attention_mask = torch.ones(language_model_inputs.size()[:-1],
                                             dtype=torch.long,
                                             device=language_model_inputs.device)
        if input_ids is None:
            input_ids = (torch.LongTensor([[self.config.text_config.bos_token_id]]).repeat(batch_size,
