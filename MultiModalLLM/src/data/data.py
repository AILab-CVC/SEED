...
def preprocess_interleaved(sample, tokenizer, clip_processor, sim_threshold):
    ...
    num_images = torch.count_nonzero(text_tensor["input_ids"] == tokenizer.additional_special_tokens_ids[
...
def get_mimicit_dataset(args, tokenizer, epoch=0, floor=False):
    ...
    args.train_num_samples = (sum(len(dataset) for dataset in unified_datasets) /
                              len(unified_datasets) if args.train_num_samples is None else args.train_num_samples)
    round_fn = math.floor if floor else math.ceil
