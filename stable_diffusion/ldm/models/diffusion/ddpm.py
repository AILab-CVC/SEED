...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.cond_stage_key in ["image", "LR_image", "segmentation", 'bbox_img'
                ...
                patch_limits = [(x_tl, y_tl, rescale_latent * ks[0] / full_img_w, rescale_latent * ks[1] / full_img_h)
    ...
    def progressive_denoising(self,
            ...
            if isinstance(cond, dict):
                cond = {
                    key: cond[key][:batch_size] if not isinstance(cond[key], list) else list(
                        map(lambda x: x[:batch_size], cond[key]))
                    for key in cond
                }
            else:
        ...
        iterator = tqdm(reversed(range(0, timesteps)), desc='Progressive Generation', total=timesteps) if verbose else reversed(
            range(0, timesteps))
